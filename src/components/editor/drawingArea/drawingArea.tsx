...
const DrawingArea = () => {
  ...
  useEffect(() => {
    ...
    if (canvasCtx && canvas.current) {
      ...
      canvasCtx.lineWidth = lineWidth; // set inital line width
      ...
      if (artId) {
        ...
        getOneArt(artId)
          .then((art) => {
            ...
            image.onload = function () { // needed because of not always image can render at time
  ...
  const onMouseDown = (e: React.MouseEvent<HTMLCanvasElement, MouseEvent>): void => {
    // Different tools requires defferent params, so here all needed params for
